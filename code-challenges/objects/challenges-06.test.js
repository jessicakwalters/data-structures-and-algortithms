...
const hasChildrenValues = (arr, character) => {
  ...
  arr.forEach((person) => {
    ...
    //check to see if it's the character
...
const totalCharacters = (arr) => {
  ...
  arr.forEach( (person => {
      let values = Object.values(person);
      //increment counter for the person
      counter++;
...
const houseSize = (arr) => {
  ...
  arr.forEach( (character) => {
    let newHouse1 = {house: character.house, members: 0};
        sizes.push(newHouse1);
        newHouse1.members++;
        newHouse1.members += character.children.length;
        if(character.spouse){
          newHouse1.members++;
        }
  })
...
const houseSurvivors = (arr) => {
  ...
  arr.forEach( (character) => {
    let newHouse1 = {house: character.house, members: 0};
        survivors.push(newHouse1);
        newHouse1.members++;
        newHouse1.members += character.children.length;
        if(!deceasedSpouses.includes(character.spouse) && character.spouse){
          newHouse1.members++;
        }
  })
  return survivors;
