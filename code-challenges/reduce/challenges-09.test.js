...
const countNumberOfChildren = (arr) => {
  let numChildren = arr.reduce( (acc, curr) => {
    let length = Object.keys(curr).length;
    if (length === 4) {
      let kids = curr['children'].length;
      return acc + kids;
    }
    return acc;
  }, 0)
...
const extractChildren = (arr) => {
  ...
  let children=[];
  let results = arr.filter( (curr) => {
    if(curr.name.includes("a")){
      return curr;
    } 
  })
